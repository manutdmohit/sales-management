...
export function DashboardClient({
  ...
  const [profitSnapshot, setProfitSnapshot] = useState<ProfitSnapshot | null>(
    initialData?.profit
      ? {
          total: initialData.profit.total,
          from: initialData.profit.from,
          to: initialData.profit.to,
        }
      : null
  );
  ...
  }, [businessId, profitPeriod, initialBusinessId]);

  const lowStock = summary.filter((s) => s.isLowStock).length;
